...
public class TurretRotation extends Enemy{
    ...
    public TurretRotation(Tile t,Level level){
            ...
            if(Attacking){
                ...
                if(AttactTimer >= AttackDuration){
                    ...
                    if(level.getCollisionPlayer(points) != -1){
                        level.projectiles.add(new Projectile(x,y,ray.Angle+Math.PI/25,7,15,false,level));
                        level.projectiles.add(new Projectile(x,y,ray.Angle,7,15,false,level));
                        level.projectiles.add(new Projectile(x,y,ray.Angle-Math.PI/25,7,15,false,level));
                    }
                }
            }
