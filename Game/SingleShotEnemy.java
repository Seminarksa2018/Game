...
public class SingleShotEnemy extends Enemy{
    ...
    public SingleShotEnemy(Tile t,Level level){
        super(t.X,t.Y,level,"SingleShot");
        Health = 300;
    }
            ...
            if(Attacking){
                ...
                if(AttactTimer >= AttackDuration){
                    ...
                    if(level.getCollisionPlayer(points) != -1){
                        level.projectiles.add(new Projectile(x,y,ray.Angle,9,damage,false,level));
                    }
                }
            }
    ...
    @Override
    public void Render() {
        level.game.drawSprite(level.SingleShot,x,y,0,0);
    }
